...
- **Trade review — championships (Tier 1) + title-contribution flag.** Detects
  ...
  stable manager. A trade is flagged **🏆 TITLE** (highlights, index, and
  ...
  each showing **when the trade was made** (which can predate the title it fed)
  and its **title-season-only** PAR/game, games (distinct fantasy weeks, from
  the trade forward), and key acquisitions — so every 🏆 tag has a stated
  "why." Titles are intentionally kept out of the manager overview, which grades
